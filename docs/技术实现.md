# NCCN / CSCO 淋巴瘤指南 RAG Agent 技术实现

> 本文档描述**当前代码**的实现方式。数据字段与溯源路径见 [`数据组织.md`](./数据组织.md)；环境、模型与索引部署见 [`环境配置.md`](./环境配置.md)。

## 概述

Guideflow 是一个面向淋巴瘤指南的 RAG 问答原型，提供 **CLI** 与 **Web** 两种入口。支持 **NCCN** 与 **CSCO** 两套并列数据源（前端下拉框逐条消息选择；单次检索不混用）。核心思路：
...
**检索路径为 BM25-only**。NCCN 流程图类问题可通过 `view_pages` 按需渲染 PNG 交 VLM；**CSCO 关闭 VLM / flowchart / KG**，仅纯文本（叙述 + 表格 Markdown）。

## 目标与非目标

| 目标 | 非目标 |
|------|--------|
| 验证 NCCN / CSCO 指南 RAG 全链路 | 替代临床判断 |
| 覆盖临床指南页 + Discussion / CSCO 章节表 | 向量语义检索 / hybrid 向量融合 |
| 中文问题 + 英文检索扩展 | 生产级多租户 / 高并发 |
| 智能体按需检索；NCCN 可看图、查图谱 | 离线预渲全部流程图 |
| 意图路由：指南证据 / 通用医学（标注）/ 闲聊 | 把通用医学回答伪装成指南证据 |
| 多轮追问与抗话题污染；双源逐条切换 | 同一检索请求混用两源证据 |
| 流程图问题图文并茂（NCCN 按需） | — |
...
**多源注册**：`backend/api/server.py` 的 `get_qa_service(data_source)`；`AskRequest.data_source ∈ {nccn,csco}`。  
...
scripts/                    CLI 入口
  ...
  build_bm25_index.py       知识库 → BM25（--source nccn|csco）
...
backend/app/
  ...
  prompts.py                集中提示词（含 CSCO 变体与证据图例）
  services/
    ...
    disease_scope.py        多病种 scope（NCCN + CSCO 注册表）
