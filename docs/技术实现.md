# NCCN / CSCO / EHA 淋巴瘤指南 RAG Agent 技术实现

> 本文档描述**当前代码**的实现方式。数据字段与溯源路径见 [`数据组织.md`](./数据组织.md)；环境、模型与索引部署见 [`环境配置.md`](./环境配置.md)。

## 概述

Guideflow 是一个面向淋巴瘤指南的 RAG 问答原型，提供 **CLI** 与 **Web** 两种入口。支持 **NCCN、CSCO、EHA** 三套并列数据源（前端下拉框逐条消息选择；单次检索不混用）。核心思路：

1. **离线**：把各源 PDF 解析为结构化知识库，构建**独立** BM25 索引；NCCN 另可构建知识图谱。**无向量索引步骤。**
2. **在线（默认 agentic）**：按 `data_source` 选择对应 `QAService` → 多轮历史改写 → 病种自动识别 → **智能体工具循环** → 生成。

**检索路径为 BM25-only**。NCCN 流程图类问题可通过 `view_pages` 按需渲染 PNG 交 VLM；**CSCO / EHA 关闭运行时 VLM / flowchart / KG**，仅纯文本（叙述 + 表格 Markdown）。EHA 表格在**构建期**用 VLM 转 Markdown 落库。

## 多源能力对照

| 能力 | NCCN | CSCO | EHA |
|------|------|------|-----|
| PDF 解析 | `pdf_extractor.py` | `csco_extractor` + `csco_table_extractor` | `paper_extractor` + `paper_table_vlm`（构建期 VLM） |
| 表格 | 流程图/方案表页 → 运行时 VLM 读图 | 预抽 Markdown → 检索 + UI 展示 | 构建期 VLM 转 Markdown → 检索 + UI 展示（`content_type=table`） |
| Agent 工具 | 4 个全开 | 仅 `search_guidelines` + `respond_directly` | 同 CSCO（仅 search + respond_directly） |
| KG | 默认启用 | 默认关闭 | 默认关闭 |
| 运行时 VLM / flowchart | 启用 | 关闭 | 关闭 |

> 表格渲染通道是全源通用的（`content_type=table` → `table_markdown` → 前端表格卡片）；抽取器按源不同。表格**不是** Agent 工具，也不是 LLM 现场编表。

## 目标与非目标

| 目标 | 非目标 |
|------|--------|
| 验证 NCCN / CSCO / EHA 指南 RAG 全链路 | 替代临床判断 |
| 覆盖临床指南页 + Discussion / CSCO·EHA 章节表 | 向量语义检索 / hybrid 向量融合 |
| 中文问题 + 英文检索扩展 | 生产级多租户 / 高并发 |
| 智能体按需检索；NCCN 可看图、查图谱 | 离线预渲全部流程图 |
| 意图路由：指南证据 / 通用医学（标注）/ 闲聊 | 把通用医学回答伪装成指南证据 |
| 多轮追问与抗话题污染；多源逐条切换 | 同一检索请求混用多源证据 |
| 流程图问题图文并茂（NCCN 按需） | — |
| 可选联网 PubMed **摘要级**二级文献（`[Ln]`） | 全文下载 / Europe PMC / Unpaywall；文献进 `[Sn]` 医学判断 |
...
┌─────────────────────────────────────────────────────────────────┐
│                        离线构建                                  │
│  NCCN PDF ──► pdf_extractor ──► dlbcl_knowledge_base.json       │
│                    └──► bm25_index.pkl (+ 可选 KG)               │
│  CSCO PDF ──► csco_extractor ──► csco_knowledge_base.json       │
│                    └──► csco_bm25_index.pkl（无运行时 VLM/KG）   │
│  EHA  PDF ──► paper_extractor (+ paper_table_vlm)               │
│                    └──► eha_knowledge_base.json / eha_bm25…     │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              在线问答（默认 routing_mode: agentic）               │
│  问题(+history, data_source, enable_literature)                  │
│       ──► qa_services[source] 惰性加载                           │
│       ──► 改写/话题切换 ──► 病种识别（按源注册表）               │
│       ├─► [可选并行] PubMed 摘要检索 → LiteratureHit[Ln]         │
│       └─► AgentOrchestrator（按源过滤工具）                      │
│              ├─ search_guidelines  → BM25 → rerank → 门控        │
│              ├─ query_graph / view_pages（仅 NCCN）              │
│              └─ respond_directly   → 闲聊/通用医学早退出         │
│       ──► 引用展开 ──► 生成(+历史) ──► 追加文献附录 ──► 输出     │
└─────────────────────────────────────────────────────────────────┘
```

**编排入口**：`backend/app/services/qa.py` 中的 `QAService.ask()` / `ask_stream()`。  
**多源注册**：`backend/api/server.py` 的 `get_qa_service(data_source)`；`data_source` 取自 `list_source_keys()`（当前 `nccn|csco|eha`）。  
**配置**：`config.yaml` 的 `sources:` / `literature:` + `backend/app/settings.py` 的 `settings_for_source()`。

## 代码结构

```text
scripts/                    CLI 入口
  build_knowledge_base.py   NCCN PDF → 知识库
  build_csco_knowledge_base.py  CSCO PDF → 知识库
  build_paper_knowledge_base.py 论文型指南（EHA）→ 知识库
  build_bm25_index.py       知识库 → BM25（--source 动态）
  build_knowledge_graph.py  知识库 → 知识图谱（可选，NCCN）
  import_kg_to_neo4j.py     JSON 图谱 → Neo4j（可视化 schema，可选）
  ask.py                    问答
  inspect_retrieval.py      仅检索调试

backend/app/
  models.py                 数据模型、GraphTriple、QAResult、to_web_payload
  settings.py               配置（paths、sources、retrieval、routing 等）
  prompts.py                集中提示词（含 CSCO / EHA 变体与证据图例）
  services/
    pdf_extractor.py        NCCN PDF 解析
    csco_extractor.py       CSCO PDF 解析
    csco_table_extractor.py CSCO 表格 → Markdown
    paper_extractor.py      论文型指南解析（PaperProfile 驱动）
    paper_table_vlm.py      构建期 VLM 表格转录 + 磁盘缓存
    store.py / bm25_store.py  知识库与 BM25
    knowledge_graph.py      图谱构建 / 本体
    kg_retriever.py         图谱检索（JSON）
    neo4j_graph_service.py  邻域 API + APOC 导入（可视化）
    neo4j_importer.py       可选完整图导入（与上勿混用）
    agent_orchestrator.py   智能体工具循环（按源过滤工具）
    agent_tools.py          工具 schema、status 事件、AgentState
    query_normalizer.py     问题归一化
    disease_scope.py        多病种 scope（NCCN + CSCO + EHA 注册表）
    retrieval.py            路由标签、Bm25Retriever（HybridRetriever 别名）
    reranker.py             LexicalReranker（默认）
    reference_resolver.py   句子级参考文献关联
    literature_query.py     中文问题 → PubMed 检索式（Q2Q）
    pubmed_client.py        NCBI ESearch / EFetch + TTL 缓存
    literature_ranker.py    摘要级本地加权重排
    literature_service.py   阶梯召回编排（并行分支）
    literature_format.py    `[Ln]` 附录与中性概述
...
- 在线由智能体工具 `query_graph` 或 linear 模式固定步骤调用 `KnowledgeGraphRetriever`（读 JSON，**不连 Neo4j**）；图谱文件缺失时降级为空图，不阻断问答。
- 可选可视化：`neo4j_graph_service.py` + `/api/graph/neighborhood`（需 APOC；导入用 `scripts/import_kg_to_neo4j.py` 或 `build_knowledge_pipeline`）。另有 `neo4j_importer.py`（完整 Page/Chunk schema），与可视化导入**勿混用**。
...
- **病种护栏**（`_reinject_disease_hits`）：门控后若某具体病种问句只剩公共模块页（如仅 `NHODG-A`），从门控前候选回补 1–2 条同病种高分页（`disc-<病种>-*` / 本病种模块），避免「BM25 命中对了、门控却塌缩到公共页」。
- 可通过 `config.yaml` 的 `enable_evidence_gating: false` 关闭。

Trace 事件：`evidence_gated`（含 `disease_guard` 是否触发）
...
**模块**：`kg_retriever.py` · `disease_scope.py`（来源前缀→病种解析）· `qa.py`（子集过滤）

**何时"检索"图谱**：

- **Agentic**：智能体调用 `query_graph` 时执行（结果已按当前病种 scope 过滤）；若其 `graph_hits < 2`，由 `qa._retrieve_graph_fallbacks` 补召回（KG JSON 传入 scope + `min_relevance=0.12`，可选叠加 Neo4j 边）。
- **Linear**：每次 guideline 问答固定检索一次（同样传入 scope）。

步骤：实体归一化 → 多跳扩展 `GraphTriple` → **按病种 scope 过滤** → 拆成「UI 份」与「prompt 份」两路。
...
| 份 | 内容 | 去向 |
|----|------|------|
| `graph_triples`（全量 / UI） | 检索结果 + Neo4j 边 + 为凑够 2 条而合成的 synth 边 | `QAResult.graph_triples`、前端图谱面板、trace —— **每次都能看到** |
| `graph_triples_prompt`（子集 / 证据） | 经「病种 + 质量」闸门过滤后的子集 | `EvidenceBundle.graph_triples` → `[G*]` prompt 段 —— **只有它影响答案** |

> `build_evidence_prompt` 只读 `EvidenceBundle.graph_triples`；子集为空时 `[G*]` 段自然不输出。生成收尾处「凑够 2 条」的 Layer C 发生在**生成之后**，只补 UI 份，不回写 prompt。

#### 图谱证据（`[G*]`）的触发条件

一条三元组要进入 prompt、真正**影响答案**，必须**同时**满足：

1. **可作证据**：`subject_name`、`object_name` 均非空；且**不是**凑数边——`triple_id` 不以 `synth:` / `fallback:` 开头，且 `review_status ≠ synthetic`、`validation_status ≠ fallback`。（Neo4j 空名边在检索阶段即丢弃。）
2. **命中病种 scope**（`triple_sources_in_scope`，解析 `evidence_source_ids` 前缀 → article / module）：
   - **具体病种**：至少一个来源解析到**本病种 article**（如 `disc-dlbcl-*` / `ref-dlbcl-*`）或**本病种/公共模块**（如 `page-BCEL-*`，公共 `NHODG / DIAG / ABBR / ST / CAT`、`overview`）；
   - 来源**全部**解析为其他病种（如 `disc-mzl-*` / `ref-hgbl-*`）→ **丢弃**；
   - 来源**无法解析**（如裸 Neo4j edge id）→ 需 `subject/object` 名称命中病种词或问题实体，否则只留 UI 份；
   - `scope=all`（问题未写病名）→ 不做病种过滤，但仍剔除 synth / 空名边。
3. **子集非空**才输出 `[G*]`；否则答案完全基于 `[Sn]`（与无图谱行为一致）。

> 一句话：**图谱只在「存在真正属于当前病种的可信三元组」时才参与作答**；跨病种、合成、空名等噪声只用于观测，不进 prompt。这样即便智能体没显式调用 `query_graph`、或 KG 只有别的病种命中，也不会污染答案。

Trace 事件：`prompt_built.graph_triple_count`（进 prompt 的条数）与 `graph_triple_count_ui`（UI 全量）· `graph_payload_debug`（UI 份明细）· `reasoning_path.graph_steps`。
...
| `literature.*` | enabled_default / candidate_k / final_top_k / recent_years / timeouts / cache_dir / ncbi_* | 默认关闭；见「实时文献检索」 |
...
| Neo4j（可选，`.env`） | `NEO4J_URI` / `USER` / `PASSWORD` / `DATABASE` | 仅「Neo4j 图谱」与导入需要；驱动 `neo4j>=5.20,<6` |
...
| `evidence_gated` | 门控保留的 `[Sn]`、`disease_guard` 是否回补 |
| `prompt_built` / `graph_payload_debug` | 进 prompt 的图谱条数 vs UI 全量、跨病种是否被过滤 |
| `figures_gathered` / `figures_pruned` | seed、按需看图、裁剪 |
| `attached_references` | 句子级引用是否过宽 |
| `literature_search` | PubMed 检索式、阶梯层级、PMID 列表、降级原因 |
...
| `test_paper_extractor.py` | EHA / 论文型抽取器（章节、表格、参考文献、上标引用） |
| `test_csco_extractor.py` | CSCO 抽取与表格 Markdown |

## 当前限制

- 检索为 BM25 + 词面 rerank，语义召回依赖中→英 query 扩展与智能体多步检索。
- 未配置 `VLM_API_KEY` 时，流程图问题降级为文本证据（不读图）；EHA 构建期抽表也需要同一 Key。
...
- 知识图谱偏规则/本体抽取；图谱缺失时自动降级为空图。跨病种过滤依赖 `evidence_source_ids` 前缀约定；Neo4j 裸边（无病种来源）只能按 `subject/object` 名称匹配，故默认更保守——多数只进 UI、不进 prompt。
- 医学回答（含标注的通用医学）仅供辅助检索与科普，不替代临床判断。

## 实时文献检索（PubMed 摘要级二级源）

> **本阶段已落地**：仅 PubMed、仅摘要、请求级开关 `enable_literature`；结果以 `[L1]..[Ln]` 列入 References，**不进**主证据 prompt、不占用 `[Sn]`。  
> **仍属后续**：全文 / Europe PMC / Unpaywall、本地论文向量库、PaperQA2 级 RCS。

### 触发与边界

- API：`POST /api/ask` 字段 `enable_literature: bool`（默认 `false`）；前端「联网文献」开关持久化到 `localStorage`。
- 与指南 BM25 / Agent 工具循环**并行**发起；失败写 `degraded: literature_unavailable`，**不阻断**主回答。
- 回答末尾追加独立小节「指南外最新文献（供参考）」；每条标注「PubMed · 仅摘要 · 未经指南收录」。

### 算法流水线

1. **Q2Q**（`literature_query.py`）：Qwen 概念抽取（失败则规则表）→ 疾病固定 MeSH 词表 + `[MeSH Terms]`/`[tiab]` 双写检索式（绕开 ATM，并覆盖尚未标 MeSH 的新文献）。
2. **阶梯 ESearch**（`literature_service.py`）：L1 严格（近 5 年）→ 过少则 L2/L3 放宽；过多则追加 Meta-Analysis/RCT/Guideline 过滤。`sort=relevance`（PubMed Best Match）。
3. **EFetch 摘要**（`pubmed_client.py`）：批量 XML；`AbstractText` 按 `Label` 拼接；两层 TTL 缓存（query 1 天 / article 30 天）。
4. **本地重排**（`literature_ranker.py`）：`0.35*rel + 0.25*design + 0.20*recency + 0.15*topic - penalty`；硬过滤未出现关键 biomarker/intervention 的候选；取 `literature.final_top_k`（默认 5）。

### Trace / 冒烟

- Trace 事件：`literature_search`（含 `final_term`、ladder、pmids）。
- 脚本：`python scripts/smoke_literature.py`（对 `docs/测试问题.md` 抽样题跑召回）。
- 单测：`tests/test_literature.py`。

### 后续选型（全文入库时）

来自横向评测与 PaperQA2 的结论仍适用：切分策略优先；解析器可考虑 Docling；检索层可升级为带 LLM 打分的 RCS；指南与论文需 evidence tier，避免论文压过指南。
