...
**检索路径为 BM25-only**：不加载 embedding 模型，不维护向量库。流程图类问题通过智能体工具 `view_pages` **按需渲染** PNG，交给 VLM 看图回答。知识图谱由智能体按需调用 `query_graph` 补召回与路径推理；指南内回答仍以 `[Sn]` 为准，图谱仅在**命中当前病种的可信三元组**时才以 `[G*]` 辅助（详见「知识图谱检索」）。
