...
│  问题(+history, data_source) ──► qa_services[source] 惰性加载    │
│       ──► 改写/话题切换 ──► 病种识别（按源注册表）               │
...
**配置**：`config.yaml` 的 `sources:` + `backend/app/settings.py` 的 `settings_for_source()`。
...
> Roadmap，**非本轮实现**。一级数据源是指南（NCCN / CSCO / EHA）；未来海量论文作为二级源补充。

来自 2026 年横向评测（arXiv 2604.04948）与 PaperQA2 的结论：

- **切分策略比解析器更重要**。同一解析器下「层级切分 + 元数据富化」把 QA 准确率从约 74.7% 拉到 94.1%；表格类问题在基础切分与层级切分之间可差约 33 个百分点。当前 `paper_extractor` 保留 section 层级、把表格独立成 `content_type=table` chunk，方向正确。
- **解析器优先选 Docling**（IBM，已捐给 LF AI & Data）：布局分析与表格结构识别强，输出 `DoclingDocument` 带层级，自带 `HybridChunker`。备选 MinerU（公式强、需 GPU）、Marker（吞吐快、复杂多栏偏弱）。GROBID 不做全文转换，但做**参考文献与元数据抽取（TEI）**仍最成熟，可与 Docling 组合。
- 上量时把规则式 backend 换成 Docling，**保留 `PaperProfile` 接口不变**——抽取器已按「profile 驱动 + 可替换 backend」设计。
- **检索层参考 PaperQA2 的 RCS**（Retrieval → Contextual Summarization → Rerank）：对每个候选 chunk 先做 query-aware 摘要再打分过滤，只把高分摘要注入 prompt。现有 `gate_evidence` 是同一思路的轻量版，扩到论文规模时值得升级成带 LLM 打分的 RCS。
- **谨慎扩知识图谱**：同评测里 GraphRAG 反而更差（约 82% vs 94.1%）。现有 KG 只在 NCCN 默认开启；论文源建议先不上。
- **证据分级**：一级指南与二级论文应在 `SearchDocument` 上带 evidence tier，生成时明确「指南优先、论文补充」，避免论文结论压过指南推荐。元数据可用 Crossref / Semantic Scholar 富化（引用数、期刊、撤稿状态）做质量加权。
